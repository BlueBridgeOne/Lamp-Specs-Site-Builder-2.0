	...
	,	register: function (user_data)
		{
			...
			else
			{
				...
				res=ModelsInit.session.registerCustomer({
					firstname: user_data.firstname
				,	lastname: user_data.lastname
				,	companyname: user_data.company
				,	email:user_data.email
				...
				var customerUpdate = {
					...
					customfields: {
						custentity_bb1_sb_subscribe: (user_data.emailsubscribe && user_data.emailsubscribe !== 'F') ? 'T' : 'F',
						custentity_bb1_sb_unsubscribe: (user_data.emailunsubscribe && user_data.emailunsubscribe !== 'F') ? 'T' : 'F'
					}
				};
