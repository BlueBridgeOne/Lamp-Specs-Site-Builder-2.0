...
define(
	'Wizard.View', [
		'wizard.tpl', 'Backbone', 'Backbone.CompositeView', 'Wizard.StepNavigation.View'
	],
	function(
		wizard_tpl, Backbone, BackboneCompositeView, WizardStepNavigationView
	) {
		'use strict';
		...
		return Backbone.View.extend({
			...
			initialize: function(options) {
				...
				//BB1 Closed 3D secure modal so show buttons again on review page:
				var self = this;
				jQuery(document).on('hidden.bs.modal', function() {
					// enable navigation buttons
					if (self.currentStep.getName() == "Review Your Order") {
						self.wizard.getCurrentStep().enableNavButtons();
						self.wizard.getCurrentStep().changeLabelContinue();
						// enable inputs and buttons
						jQuery('input, button,select').prop('disabled', false);
