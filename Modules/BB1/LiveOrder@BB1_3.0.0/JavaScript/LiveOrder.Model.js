...
// @extends Order.Model
define('LiveOrder.Model', ['Order.Model', 'OrderLine.Model', 'OrderLine.Collection', 'Session', 'Tracker', 'Singleton', 'Profile.Model', 'AjaxRequestsKiller', 'SC.Configuration', 'Utils', 'underscore', 'jQuery', 'OrderHistory.Model', 'GlobalViews.Message.View'], function(
	OrderModel, OrderLineModel, OrderLineCollection, Session, Tracker, Singleton, ProfileModel, AjaxRequestsKiller, Configuration, Utils, _, jQuery, OrderHistoryModel, GlobalViewsMessageView
) {
	'use strict';
	...
	var LiveOrderModel = OrderModel.extend({
		...
		initialize: function(attributes) {
			...
			this.on('change:confirmation', function(model, confirmation) {
				model.set('confirmation', new OrderHistoryModel(confirmation), {
					silent: true
				});
			});
		...
		submit: function() {
			...
			var invoice, paypal;
			if (paymentmethods) {
				invoice = paymentmethods.findWhere({
					type: 'invoice'
				});
				paypal = paymentmethods.findWhere({
					type: 'paypal'
				});
			}
			if (paypal || invoice) {
				...
				var self = this,
					creditcard = this.get('paymentmethods').findWhere({
						type: 'creditcard'
					});
		...
		placeOrder: function() {
			...
			jQuery.getJSON('bb1_lampspecs_placeorder.ss?internalid=cart&c=' + SC.ENVIRONMENT.companyId + "&n=" + SC.ENVIRONMENT.siteSettings.siteid + '&t=' + new Date().getTime(), this.placeOrderCallBack);
		},
		placeOrderCallBack: function(responseObj) {
			...
			if (responseObj.statuscode && responseObj.statuscode == "error") {
				...
				if (responseObj.reasoncode && responseObj.reasoncode == "ERR_WS_REQ_PAYMENT_AUTHORIZATION") {
					if (responseObj.paymentauthorization) {
						...
						view.title = _('Verify Payment').translate();
						//responseObj.paymentauthorization.servicehtml="<button onclick=\"window.top.location='#confirmation?force=true&last_order_id=';\">Test</button>";
						view.render = function() {
							this.$el.append('<div id="3D Secure">' + responseObj.paymentauthorization.servicehtml + '</div>');
						};

						SC._applications.Checkout.getLayout().showInModal(view);
			...
			if (error) {
				var $error = jQuery('<div>').append(error);
				...
				jQuery('[data-type="alert-placeholder-step"]').html(global_view_message.render().$el.html());

				jQuery('body').animate({
					scrollTop: jQuery('body .global-views-message-error:first').offset().top
				}, 600);

				jQuery(document).trigger('hidden.bs.modal');
			}
		}
